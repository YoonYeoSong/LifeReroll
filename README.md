...
`/api/housing/notices`는 서버에서만 공공데이터포털의 LH 공고 조회 서비스를 호출하며, 한 번에 최대 3건만 가져오고 한 시간 동안 캐시합니다. 키가 없거나 제공처 호출이 실패하면 기존 fixture를 반환합니다. 실제 공고의 자격·공급 조건은 아직 추천 엔진에 자동 적용하지 않으며, 원문 공고에서 검증한 뒤 후속 단계에서 연결합니다.
