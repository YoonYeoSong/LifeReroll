...
export function ProfileWizard() {
  ...
  const toggleAllRegions = () => set("preferences", { ...profile.preferences, preferredRegions: allRegionsSelected ? [] : [...regions] });
  ...
  return <div className="wizard"><ol className="wizard-steps">{steps.map((label, index) => <li className={index === step ? "is-current" : index < step ? "is-done" : ""} key={label}>{index + 1}. {label}</li>)}</ol><section className="housing-card wizard-card">
    ...
    {step === 4 && <><h2>희망 조건</h2><p className="field-help">희망 지역을 고르면 해당 지역의 공고만 플랜에서 추려 보여드려요. 전체 선택 시 전국 최신 공공분양 공고를 조회합니다.</p><div className="region-picker-actions"><button type="button" className="housing-secondary" onClick={toggleAllRegions} aria-pressed={allRegionsSelected}>{allRegionsSelected ? "전체 해제" : "전체 선택"}</button><span>{allRegionsSelected ? "전국 공고 조회" : `${profile.preferences.preferredRegions.length}개 지역 선택`}</span></div><div className="region-picker" role="group" aria-label="희망 지역 선택">{regions.map(region => <label key={region}><input type="checkbox" checked={profile.preferences.preferredRegions.includes(region)} onChange={() => updateRegion(region)} /><span>{region}</span></label>)}</div><div className="housing-form-grid"><MoneyField label="최대 분양가" value={profile.preferences.maxPrice} onChange={value => set("preferences", { ...profile.preferences, maxPrice: number(value) })} help="부대비용을 제외한 분양가 기준으로 입력하세요." /></div></>}
