...
export function ProfileWizard() {
  ...
  useEffect(() => {
    ...
    try { queueMicrotask(() => setProfile({ ...emptyProfile, ...JSON.parse(savedProfile) })); }
  ...
  return <div className="wizard"><ol className="wizard-steps">{steps.map((label, index) => <li className={index === step ? "is-current" : index < step ? "is-done" : ""} key={label}>{index + 1}. {label}</li>)}</ol><section className="housing-card wizard-card">
    ...
    {step === 3 && <><h2>소득·자금</h2><p className="field-help">부담 수준을 가늠하기 위한 간단한 자금 계획입니다. 금액이 없거나 모르면 비워두세요.</p><div className="housing-form-grid">{textField("연소득 (원)", profile.income.annualIncome, value => set("income", { ...profile.income, annualIncome: number(value) }), "number")}{textField("근속기간 (개월)", profile.income.employmentPeriod, value => set("income", { ...profile.income, employmentPeriod: number(value) }), "number")}{textField("바로 쓸 수 있는 현금 (원)", profile.assets.availableCash, value => set("assets", { ...profile.assets, availableCash: number(value) }), "number")}{textField("예금·적금 및 금융자산 (원)", profile.assets.savings + profile.assets.financialAssets, value => set("assets", { ...profile.assets, savings: number(value), financialAssets: 0 }), "number")}{textField("가족 증여 예정액 (원)", profile.familySupport.gift?.amount, value => set("familySupport", { ...profile.familySupport, gift: { amount: number(value), previousGiftAmount: profile.familySupport.gift?.previousGiftAmount ?? 0 } }), "number")}{textField("가족 차용 예정액 (원)", profile.familySupport.loan?.amount, value => set("familySupport", { ...profile.familySupport, loan: { amount: number(value), interestRate: profile.familySupport.loan?.interestRate ?? 0, repaymentYears: profile.familySupport.loan?.repaymentYears ?? 0, monthlyRepayment: profile.familySupport.loan?.monthlyRepayment ?? 0 } }), "number")}</div><p className="funding-preview">현재 입력 자금: 자기자금 {formatWon(profile.assets.availableCash + profile.assets.savings + profile.assets.financialAssets)} · 증여 {formatWon(profile.familySupport.gift?.amount ?? 0)} · 차용 {formatWon(profile.familySupport.loan?.amount ?? 0)}</p></>}
    {step === 4 && <><h2>희망 조건</h2><p className="field-help">희망 지역을 고르면 해당 지역의 공고만 플랜에서 추려 보여드려요.</p><div className="region-picker" role="group" aria-label="희망 지역 선택">{regions.map(region => <label key={region}><input type="checkbox" checked={profile.preferences.preferredRegions.includes(region)} onChange={() => updateRegion(region)} /><span>{region}</span></label>)}</div><div className="housing-form-grid">{textField("최대 분양가 (원)", profile.preferences.maxPrice, value => set("preferences", { ...profile.preferences, maxPrice: number(value) }), "number", "부대비용을 제외한 분양가 기준으로 입력하세요.")}{textField("희망 면적 (㎡)", profile.preferences.preferredHousingSizes[0], value => set("preferences", { ...profile.preferences, preferredHousingSizes: value ? [number(value)] : [] }), "number")}</div></>}
