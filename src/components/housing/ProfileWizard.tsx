...
export function ProfileWizard() {
  ...
  const allRegionsSelected = regions.every(region => profile.preferences.preferredRegions.includes(region));
  const selectAllRegions = () => set("preferences", { ...profile.preferences, preferredRegions: [...regions] });
  const clearRegions = () => set("preferences", { ...profile.preferences, preferredRegions: [] });
  ...
  return <div className="wizard"><ol className="wizard-steps">{steps.map((label, index) => <li className={index === step ? "is-current" : index < step ? "is-done" : ""} key={label}>{index + 1}. {label}</li>)}</ol><section className="housing-card wizard-card">
    ...
    {step === 4 && <><h2>희망 조건</h2><p className="field-help">전국 공고를 보고 싶다면 아래 <b>전체 지역</b> 버튼만 누르세요. 특정 지역만 보고 싶을 때에만 목록에서 지역을 고르면 됩니다.</p><div className="region-picker-actions"><button type="button" className="housing-primary" onClick={selectAllRegions} aria-pressed={allRegionsSelected}>전체 지역</button>{profile.preferences.preferredRegions.length > 0 && <button type="button" className="housing-secondary" onClick={clearRegions}>선택 해제</button>}<span>{allRegionsSelected ? "전국 공고 조회" : profile.preferences.preferredRegions.length ? `${profile.preferences.preferredRegions.length}개 지역 선택` : "지역을 선택하지 않으면 현재 거주 지역을 조회"}</span></div><div className="region-picker" role="group" aria-label="희망 지역 선택">{regions.map(region => <label key={region}><input type="checkbox" checked={profile.preferences.preferredRegions.includes(region)} onChange={() => updateRegion(region)} /><span>{region}</span></label>)}</div><div className="housing-form-grid"><MoneyField label="최대 분양가" value={profile.preferences.maxPrice} onChange={value => set("preferences", { ...profile.preferences, maxPrice: number(value) })} help="부대비용을 제외한 분양가 기준으로 입력하세요." /></div></>}
    {step === 5 && <><h2>분석 준비 완료</h2><p className="page-intro">입력한 조건을 기준으로 현재 LH 공식 공고를 추리고, 자금 부담 수준과 확인할 조건을 함께 보여드릴게요.</p><button className="housing-primary" onClick={finish}>내 조건으로 플랜 보기</button>{saved && <p className="success-note">입력값을 이 브라우저에 저장했습니다.</p>}</>}
