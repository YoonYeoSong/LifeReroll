...
function PlanCard({ item }: { item: Recommendation }) {
  ...
  const priceLabel = isOfficialNotice ? "공고상 평균 분양가" : "예상 분양가";

  return <article className="housing-card result-card">
    <div className="result-title"><div><p className="housing-kicker">{isOfficialNotice ? "LH OFFICIAL SALE" : "EXAMPLE SALE"} · {item.notice.region} {item.notice.city}</p><h3>{item.notice.title}</h3><p className="application-date">타입 {typeLabel}{supplyText} · 접수 {item.notice.applicationStartDate} ~ {item.notice.applicationEndDate}</p></div><span className={`status status-${item.status.toLowerCase()}`}>{fundingLabels[item.funding.status]}</span></div>
    ...
    <p className="source-line">{isOfficialNotice ? <>공고상 평균 분양가를 읽어온 <b>참고용 대출 시뮬레이션</b>입니다.</> : <>이 카드는 <b>예시 분양 정보에 대한 시뮬레이션</b>입니다.</>} 실제 공고·분양가·대출 조건은 <a href={item.notice.sourceUrl} target="_blank" rel="noreferrer">공식 자료</a>와 금융기관에서 확인하세요.</p>
  </article>;
}
