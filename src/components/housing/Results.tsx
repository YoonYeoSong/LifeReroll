...
export function Results() {
  ...
  const plans = [
    { key: "low", title: "예상 자금 계획이 맞는 분양", description: "입력 자금과 참고용 대출 예상치를 합산했을 때 총 예상비용을 충당하는 경우", items: allItems.filter((item) => item.funding.status === "sufficient") },
    { key: "review", title: "대출 심사·자금 확인이 필요한 분양", description: "금융기관의 실제 심사, 계약 조건 또는 일부 자금 조정이 필요한 경우", items: allItems.filter((item) => ["possible", "additional_funds_needed"].includes(item.funding.status)) },
    { key: "risk", title: "추가 자금 계획이 큰 분양", description: "참고용 예상 대출을 반영해도 자금 차이가 큰 경우", items: allItems.filter((item) => item.funding.status === "insufficient") },
...
function PlanCard({ item }: { item: Recommendation }) {
  ...
  const isOfficialNotice = item.notice.provider.includes("LH");
  return <article className="housing-card result-card"><div className="result-title"><div><p className="housing-kicker">{isOfficialNotice ? "LH OFFICIAL SALE" : "EXAMPLE SALE"} · {item.notice.region} {item.notice.city}</p><h3>{item.notice.title}</h3><p className="application-date">{isOfficialNotice ? "공급" : "예시"} 면적 {item.housingType.typeName} · 접수 {item.notice.applicationStartDate} ~ {item.notice.applicationEndDate}</p></div><span className={`status status-${item.status.toLowerCase()}`}>{fundingLabels[item.funding.status]}</span></div><section className="loan-summary" aria-label="예상 대출 요약"><p>참고용 예상 주택담보대출 범위</p><strong>{loanRange}</strong><span>기준 예상 {formatWon(item.funding.estimatedLoans.mortgage)} · 실제 승인 한도 아님</span></section><dl className="result-detail"><div><dt>{isOfficialNotice ? "공고상 평균 분양가" : "예상 분양가"}</dt><dd>{formatWon(item.housingType.price)}</dd></div><div><dt>참고용 총비용</dt><dd>{formatWon(item.housingType.estimatedTotalCost)}</dd></div><div><dt>입력 자금·지원</dt><dd>{formatWon(personalFunds)}</dd></div><div><dt>예상 추가자금</dt><dd>{item.funding.shortfall ? formatWon(item.funding.shortfall) : "없음"}</dd></div></dl><section className="financing-breakdown" aria-label="예상 자금 구성"><div><h4>참고용 자금 구성</h4><span>전부 예상치</span></div><dl><div><dt>예상 담보대출</dt><dd>{formatWon(item.funding.estimatedLoans.mortgage)}</dd></div>{plannedCreditLoan > 0 && <div><dt>입력한 신용대출 계획</dt><dd>{formatWon(plannedCreditLoan)}</dd></div>}<div><dt>예상 대출 합계</dt><dd>{formatWon(totalLoans)}</dd></div></dl><p>담보대출 범위는 {isOfficialNotice ? "공고상 평균 분양가" : "예시 분양가"}·입력 자금·기존 부채를 단순 반영한 계산입니다. 신용대출은 사용자가 직접 입력한 계획 금액일 뿐, 대출 가능 금액으로 판단하지 않습니다.</p></section><details className="eligibility-details"><summary>청약 조건 참고 보기 · {eligibilityLabels[item.status]}</summary><ul className="reason-list">{item.reasons.length ? item.reasons.slice(0, 3).map((reason) => <li key={reason}>{reason}</li>) : <li>입력한 기본 조건을 기준으로 추가 확인 항목이 없습니다.</li>}</ul><p>청약 자격과 공급 조건은 반드시 공식 공고문에서 최종 확인하세요.</p></details><p className="source-line">{isOfficialNotice ? <>공고상 평균 분양가를 읽어온 <b>참고용 대출 시뮬레이션</b>입니다.</> : <>이 카드는 <b>예시 분양 정보에 대한 시뮬레이션</b>입니다.</>} 실제 공고·분양가·대출 조건은 <a href={item.notice.sourceUrl} target="_blank" rel="noreferrer">공식 자료</a>와 금융기관에서 확인하세요.</p></article>;
}
