...
export function analyzeEligibility(profile: UserProfile, notice: HousingNotice): EligibilityAnalysis {
  ...
  if (notice.incomeRequirements) reasons.push({ label: "소득", status: profile.income.annualIncome > 0 ? profile.income.annualIncome <= notice.incomeRequirements.maxAnnualIncome ? "eligible" : "ineligible" : "unknown", detail: `연소득 기준 ${formatWon(notice.incomeRequirements.maxAnnualIncome)} 이하 조건입니다.` });
