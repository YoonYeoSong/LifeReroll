...
export function analyzeSubscription(profile: UserProfile, notice: HousingNotice): SubscriptionAnalysis {
  ...
  if (notice.subscriptionRequirements.minimumRecognizedAmount) reasons.push({ label: "청약 인정금액", status: account.recognizedPaymentAmount >= notice.subscriptionRequirements.minimumRecognizedAmount ? "eligible" : "conditionallyEligible", detail: `인정금액 ${Math.round(account.recognizedPaymentAmount / 10_000).toLocaleString()}만원을 현재잔액과 별도로 비교했습니다.` });
